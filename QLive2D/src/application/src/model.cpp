...
void Model::LoadAssets(const csmChar* dir, const csmChar* fileName)
{
    ...
    ICubismModelSetting* setting = new CubismModelSettingJson(
                reinterpret_cast<Csm::csmByte*>(buffer.data()),
                buffer.size()
                );
    buffer.clear();
    SetupModel(setting);
...
void Model::PreloadMotionGroup(const csmChar* group)
{
    ...
    for (csmInt32 i = 0; i < count; i++)
    {
        ...
        AppUtils::readFileContent(path.GetRawString(),buffer);
        CubismMotion* tmpMotion = static_cast<CubismMotion*>(LoadMotion(reinterpret_cast<Csm::csmByte*>(buffer.data()), buffer.size(), name.GetRawString()));
...
CubismMotionQueueEntryHandle Model::StartMotion(const csmChar* group, csmInt32 no, csmInt32 priority, ACubismMotion::FinishedMotionCallback onFinishedMotionHandler)
{
    ...
    if (motion == NULL)
    {
        ...
        csmSizeInt size;
        AppUtils::readFileContent(path.GetRawString(),buffer);
        motion = static_cast<CubismMotion*>(LoadMotion(reinterpret_cast<Csm::csmByte*>(buffer.data()), buffer.size(), NULL, onFinishedMotionHandler));
...
Csm::csmBool Model::TestMouse(Csm::csmFloat32 x, Csm::csmFloat32 y){
    ...
    for (csmInt32 i = 0; i < count; i++)
    {
        ...
        if(IsHit(drawID, x-offsetx, y-offsety))return true;
    }
    return false;
